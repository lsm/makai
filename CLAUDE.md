...
- `.github/PULL_REQUEST_TEMPLATE.md` is deliberately minimal: what changed and why, plus reviewer notes. Keep PR descriptions short and scannable — no section headers for a single-area change, no checklist of every command that passed. The heavier requirements (spec clauses, traceability matrix, review rounds log) live in `docs/review-process.md` and apply only to the PRs its §2 scopes.
