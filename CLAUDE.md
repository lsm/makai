...
- Zero comments in tracked `.zig` and `.ts` sources (`git ls-files '*.zig' '*.ts'`, including `build.zig`, tests, and fixtures — no special cases): no line, block, doc (`///`, `//!`), or JSDoc comments — enforced by `node scripts/check-no-comments.mjs --check` (CI), ratcheted by `scripts/no-comments-allowlist.txt` (legacy files grandfathered; entries are removed as files are cleaned, and files not on the list are enforced immediately). Exempt functional directives only: `// zig fmt: off`/`// zig fmt: on` in Zig; shebangs, `/// <reference>` and `@ts-check`/`@ts-nocheck` in file-leading single-line position only, position-independent `@ts-ignore`/`@ts-expect-error`, `biome-ignore`, `eslint-*`, `oxlint-*`, knip `@public`/`knip-ignore`, and coverage ignores (`v8`/`istanbul`/`c8`) in TypeScript. Rationale belongs in commit messages, PR descriptions, `docs/`, and tests — not comments.
