...
`makai --tui`), `make sign` (macOS: code sign the local build, see below), `make test`,
`make test-tui`, `make check` (guardrail scripts), `make clean`
...
- **`build.zig.zon` `.version` is the single source of truth for the version.** `build.zig` imports the manifest (`@import("build.zig.zon")`) and feeds one shared `version_options` module to both the `makai` CLI and the MCP bridge, so `makai --version` and the MCP handshake cannot drift from the package. Bump it together with `package.json`; the release preflight fails when they disagree or when a tag does not match. Do not reintroduce a hardcoded version literal — `zig/src/tools/makai.zig` carried `"0.0.1"` through two releases while the package said `0.2.0`.
...
- Release: tagging `v*` runs `.github/workflows/release-binaries.yml` (six targets) and `scripts/package-npm.ts` builds the `@makai/cli-<platform>` optional-dependency packages that `bin/makai.js` dispatches to. CI's cross-compile smoke job builds the non-native targets on every PR. The two macOS targets are Developer ID signed (`scripts/sign-macos.sh`) and notarized (`scripts/notarize-macos.sh`) between build and packaging, so the tarball and the npm payload carry the same signed bytes; a tag build fails rather than publishing unsigned macOS binaries when the signing secrets are absent. Setup and secrets: `docs/macos-code-signing.md`. A `preflight` job gates the whole workflow: it fails the run when `build.zig.zon` and `package.json` disagree, when a `v*` tag does not match `build.zig.zon`, or when `NPM_TOKEN` / the macOS signing secrets are missing on a tag — `v0.2.0` published its GitHub release and *then* failed on npm with `ENEEDAUTH`, which is the half-released state this prevents.
