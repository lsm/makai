...
- Makai pins: `lsm/makai` `main` @ `1413ef7` ("fix(sdk): in_reply_to-aware waiter
  ...
  (#206)") for the §13.2.6-rule-6 eviction claims, and `lsm/makai#208` for the
  §6.1/§13.4.2 teardown-guard claims — that PR IS this revision of the ledger
  for them; re-pin both to their squash merge commits on `main` when they land.
  The session-lifecycle pass itself was verified against `67ad514`
