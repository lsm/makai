...
2. #204/#210 — server enforcement gaps the spec marks `[planned]`. LANDED (first
   ...
   envelope not) removes the run, since the pair is ONE settlement delivered
   by its first frame — no re-emitted projections; the trailing `agent_end`
   ...
   truncated stream cannot settle "successfully"; run-start failures mark
