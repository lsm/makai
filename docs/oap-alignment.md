...
| envelope `sequence` | `sequence` | deviating: scope | Makai: per-direction, per-session, consumed by requests, replies, and events alike — with two frame classes on the outbound side: allocated frames draw a monotonic per-session counter, while echo replies (`session_info`, `pong`, `tool_list_response`) copy the inbound sequence verbatim and validation errors carry 0 (see #204). OAP v0.1: run-scoped, positive, contiguous, and requests/responses do not consume it. Adapters must renumber per OAP run sequence from native receive order and must not order echo replies by sequence. |
