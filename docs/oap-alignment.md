...
| envelope `sequence` | `sequence` | deviating: scope | Makai: per-direction, per-session, consumed by requests, replies, and events alike (validation errors aside, which carry 0). OAP v0.1: run-scoped, positive, contiguous, and requests/responses do not consume it. Adapters must renumber per OAP run sequence from native receive order. |
...
| payload `tool_call_id` | `tool_call_id` | aligned | Unique within a session; same scoping caveat as ACP's `(sessionId, toolCallId)`. |
...
| admission (`session.message.submit.response` before stream) | writing `agent_message` admits; acceptance implicit | deviating: no admission receipt | OAP separates "the endpoint accepted the submission" from execution; makai's acceptance is observable only through subsequent events. §13.4.1. |
| settlement (exactly one terminal per accepted run) | `agent_result` (run) / trailing `agent_end` event (stream) / terminal error | aligned | Result published before trailing `agent_end`; one settlement, not two. §13.4.2. |
| single terminal arbiter (children settle first; duplicate terminals suppressed) | run pump settles result XOR error; terminal `agent_end` held until after `agent_result` | aligned | §13.4.3. |
| `run.cancel` (run-scoped; intent ≠ settlement; races defined) | `agent_stop` — session-scoped teardown that also cancels the in-flight run | deviating: cancellation is session-scoped, not run-scoped | No mid-message run-scoped cancel in v1. Per OAP Decision 0001's consequence, session-scoped cancellation forces one foreground run per session — makai enforces exactly that (`agent_busy` on duplicate start and on message-to-processing-session). Eviction (#202) and stop share cancel semantics. |
| run statuses (`queued`/`running`/`waiting_for_input`/`cancelling`/terminals) | `AgentStatus` (starting/ready/processing/waiting_for_tool/stopping/stopped/error) | renamed + partial: statuses are session-level, not run-level | No cancelled/failed terminal distinction at the status level; failure is carried by settlement frames (§3.5), not session status. `waiting_for_tool` is makai-specific (distributed tool execution). |
...
| EOF/process exit before settlement | transport rejects pending frame waits; no fabricated result | aligned | "Failure, never success" — §13.4.4, matching the ACP ledger's process-exit rule. |
...
3. #198 — rename the `agent_start` payload key `resume_session_id` → `session_id`
