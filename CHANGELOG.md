...
- Added spec §13 "Session Lifecycle & Frame Routing (V1.1)" to [`docs/v1-sdk-agent-provider-spec.md`](docs/v1-sdk-agent-provider-spec.md) — typed identity domains (`session_id` is a correlation/container key, never a resume handle, #198; `in_reply_to` references the request envelope's `message_id` only; `sequence` is per-direction per-session), session lifecycle and ownership (multi-message by design, `agent_stop` teardown per §6.1, server eviction rights with `agent_not_found` semantics and a normative idle-TTL requirement tracked in #202), frame routing (`in_reply_to`-aware delivery planned in #201; until then concurrent runs on one explicit session id fail with `agent_busy`), admission vs settlement with a single terminal arbiter (writing `agent_message` admits; `agent_result`/trailing `agent_end`/terminal error settles; EOF before settlement is failure), and the load/resume/replay trichotomy (makai v1 has none; no field implies any). Added [`docs/oap-alignment.md`](docs/oap-alignment.md): the OAP deviations ledger (OAP term → makai construct → status) as the convergence contract for OAP adapter lsm/open-agent-protocol#3, referencing OAP Decision 0001.
