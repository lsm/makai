...
fn getVertexToken(allocator: std.mem.Allocator) !?[]const u8 {
    return test_helpers.getApiKey(allocator, "google_vertex");
}

fn getVertexProject(allocator: std.mem.Allocator) !?[]const u8 {
    if (std.process.getEnvVarOwned(allocator, "GOOGLE_VERTEX_PROJECT_ID")) |project| {
        return project;
    } else |_| {
        return null;
    }
...
test "google_vertex: basic text generation" {
    ...
    const prov = try google_vertex.createProvider(cfg, testing.allocator);
...
test "google_vertex: thinking mode" {
    ...
    const user_msg = types.Message{
        .role = .user,
        .content = &[_]types.ContentBlock{
            .{ .text = .{ .text = "What is 11 * 19? Think through it step by step." } },
        },
        .timestamp = std.time.timestamp(),
    };
...
test "google_vertex: tool calling" {
    ...
    const user_msg = types.Message{
        .role = .user,
        .content = &[_]types.ContentBlock{
            .{ .text = .{ .text = "What's the weather in Paris?" } },
        },
        .timestamp = std.time.timestamp(),
    };
...
test "google_vertex: abort mid-stream" {
    ...
    const user_msg = types.Message{
        .role = .user,
        .content = &[_]types.ContentBlock{
            .{ .text = .{ .text = "Write a long story about a pirate." } },
        },
        .timestamp = std.time.timestamp(),
    };
