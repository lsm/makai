...
pub fn parseResponse(ctx: *StreamThreadContext, reader: anytype) !void {
    ...
    try ctx.stream.push(.{ .start = .{
        .model = try ctx.allocator.dupe(u8, ctx.config.model_id),
    } });
            ...
            if (content_obj) |content| {
                ...
                for (parts.array.items) |part_val| {
                    ...
                    if (part.get("text")) |text_val| {
                        ...
                        const tool_id_for_event = try ctx.allocator.dupe(u8, tool_id);
                        try ctx.stream.push(.{ .toolcall_start = .{ .index = idx, .id = tool_id_for_event, .name = name_owned } });
