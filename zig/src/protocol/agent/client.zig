...
/// The §6.1 admission evidence for one session registration (#210 gap 7).
/// `isSessionAdmitted` reports the conjunction: an EXCLUSIVE client-
/// generated id whose own request-correlated `agent_started` was observed,
/// with the registration not since removed client-side. Correlation alone
/// is deliberately NOT sufficient — the normative teardown rule (spec §6.1)
/// says a request-correlated `agent_started` proves ownership only of the
/// registration the start created, not that the same registration still
/// occupies the id when a delayed buffered reply is finally observed (the
/// id may have been stopped/evicted and re-registered in between, §13.4.5);
/// it qualifies only with generation proof (#204). Until generation tokens
/// exist, the one sufficient stand-in is exclusivity: an id no other caller
/// could have supplied — and only a foreign re-registration of a CALLER-
/// SUPPLIED id can silently occupy the id between the start and its
/// delayed reply, so `started_observed` alone never becomes admission.
const SessionAdmission = struct {
    /// Whether the session id is EXCLUSIVE to this client — generated by
    /// this client's `sendAgentStart`, so no other caller could have
    /// supplied it or later re-registered it after a teardown this client
    /// missed. Caller-supplied ids (through `sendAgentStartWithSession`)
    /// cannot be proven exclusive by the client and stay false.
    exclusive_id: bool = false,
    /// Whether a reply correlated to this client's own tracked start was
    /// observed for the registration. Recorded for every id kind (the raw
    /// fact survives for callers that can supply their own ownership
    /// proof, §13.2.6's explicit stop); admission additionally requires
    /// `exclusive_id`.
...
pub const AgentProtocolClient = struct {
    ...
    /// Per-session §6.1 admission evidence (#210 gap 7) — see
    /// `SessionAdmission`. Reserved at every `agent_start` send, marked by
    /// a reply correlated to that start, and cleared by every teardown
    /// path — an unknown-outcome teardown stop may reconcile against the
    /// session only while the full conjunction stands (the bounded probe's
    ...
    pub fn sendAgentStart(self: *Self, config_json: []const u8, system_prompt: ?[]const u8) !agent_types.Ulid {
        ...
        // The generated id is EXCLUSIVE: no other caller could have
        // supplied it, so its admission evidence can satisfy §6.1's
        // stop-ownership bound once its own start's acceptance is observed.
        return self.startSession(sid, config_json, system_prompt, true);
    }

    /// Sends an `agent_start` for a CALLER-SUPPLIED session id. The id's
    /// exclusivity is not provable by the client (another caller may hold
    /// the same id, and after a teardown this client misses, may
    /// re-register it — §13.4.5), so a registration started this way
    /// NEVER earns `isSessionAdmitted` admission evidence on its own
    /// correlated reply alone (spec §6.1: that qualifies only with
    /// generation proof, #204). Callers that generated the id themselves
    /// and know it is exclusive hold their own positive evidence and may
    /// use the explicit-sequence stop (§13.2.6,
    ...
    fn startSession(self: *Self, sid: agent_types.SessionId, config_json: []const u8, system_prompt: ?[]const u8, id_exclusive: bool) !agent_types.Ulid {
        ...
        // mutation: the agent_started arm then marks the slot IN PLACE, so
        ...
        // attempt re-arms the evidence (nothing observed yet — the attempt
        // proves nothing on its own), so a slot left standing from an
        // earlier registration of the id cannot survive into a registration
        // the re-start is replacing — the id may have been removed and
        // re-registered by a foreign caller in between (§6.1's
            ...
            .agent_started => |p| {
                ...
                if (self.pendingSendFor(p.session_id, env.in_reply_to)) |start| {
                    if (start.kind == .start) {
                        ...
                        // names this client's OWN tracked start, so the
                        // registration THAT START CREATED was accepted —
                        // mark the raw fact on the reserved slot. A delayed
                        // or unsolicited started — from an OLDER
                        // registration on a reused id, or a FOREIGN
                        // caller's registration that won the race for a
                        // caller-supplied id — carries no tie to this
                        // client's start and settles in the adopted-start
                        // branch below unmarked. The mark alone is NOT
                        // admission: per spec §6.1 the correlation proves
                        // ownership of the registration the request
                        // created, not that the same registration still
                        // occupies the id when a delayed buffered reply is
                        // observed (it may have been stopped/evicted and
                        // re-registered in between, §13.4.5) — admission
                        // additionally requires the EXCLUSIVE generated id
                        // (see SessionAdmission), the one sufficient
                        // stand-in until generation tokens (#204). The
                        // write targets the slot RESERVED at send time; an
                        // absent slot records nothing — the safe direction,
                        // since a missing admission never arms a stop.
    ...
    fn handleCorrelatedRejection(self: *Self, session_id: agent_types.SessionId, in_reply_to: ?agent_types.Ulid, code: ?agent_types.AgentErrorCode) !void {
        ...
        // IDENTITY is deliberately NOT cleared here: the TUI surfaces this
        // rejection through the per-session error/completion bookkeeping
        // recorded by the caller arm, and its session sync keys on
        // `session_id` — nulling it here makes syncRemoteSessionFromClient
        // tear the session state down (removeSessionState) BEFORE the drain
        // pass observes that terminal state, stranding an in-flight turn
        // with no reply that can ever settle it (the pump runs sync before
        // drain). Clearing the identity moves with the TUI's
        // drain-before-sync rework (#224, gap 7 S5) (#210 gap 7).
    ...
    /// Whether the session's registration carries SUFFICIENT §6.1
    /// admission evidence (#210 gap 7): an EXCLUSIVE client-generated id
    /// (`sendAgentStart`) whose own request-correlated `agent_started` was
    /// observed, with the registration not since removed client-side
    /// (every teardown path clears the slot). An unknown-outcome teardown
    /// stop may reconcile against the session only while the full
    /// conjunction stands — a correlated reply on a caller-supplied id
    /// alone proves the registration the start created, not that it still
    /// occupies the id (spec §6.1; generation proof is #204), so
    /// `sendAgentStartWithSession` registrations never report admitted on
    /// their own (§6.1's leak-over-destroy stance).
...
test "AgentProtocolClient admission evidence requires an exclusive id's own correlated agent_started (#210 gap 7)" {
    // §6.1 admission evidence is OWNERSHIP proof, and the conjunction is
    // strict: a started with no tie to this client's own tracked start (a
    // lenient peer's uncorrelated reply, or a reply naming a FOREIGN start
    // on a reused caller-supplied id) proves nothing; and even the reply
    // correlated to this client's own start proves only the registration
    // THAT START created — on a caller-supplied id the registration may
    // have been stopped/evicted and the id re-registered by another caller
    // before a delayed buffered reply is observed (§13.4.5), so per spec
    // §6.1 it qualifies only with generation proof (#204). Only an
    // EXCLUSIVE client-generated id (sendAgentStart) plus its own
    // correlated started is sufficient pre-#204; the teardown clears it.
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    harness.wire();
    ...
    // Phase 1: a lenient peer's uncorrelated started (caller-supplied id).
    const sid1 = agent_types.generateSessionId();
    _ = try client.sendAgentStartWithSession(sid1, "{}", null); // seq 1
    try std.testing.expect(!client.isSessionAdmitted(sid1)); // nothing observed yet
    ...
    // Phase 3: the reply correlated to this client's own start on a
    // CALLER-SUPPLIED id — the raw correlation is recorded, but it is NOT
    // admission: another caller may hold or later re-register the id
    // (§13.4.5), and a teardown stop armed by this correlation alone could
    // destroy the replacement registration.
    const sid3 = agent_types.generateSessionId();
    const supplied_start_id = try client.sendAgentStartWithSession(sid3, "{}", null); // seq 1
    ...
    try std.testing.expect((client.admitted_by_session.get(sid3) orelse SessionAdmission{}).started_observed); // the fact stands
    try std.testing.expect(!client.isSessionAdmitted(sid3)); // ...but proves no exclusivity

    // Phase 4: an EXCLUSIVE client-generated id whose own correlated
    // started is observed — the sufficient pre-#204 conjunction — and the
    // agent_stopped teardown clears it with the rest.
    const start_id = try client.sendAgentStart("{}", null); // seq 1, id generated here
    var generated_start_env = try harness.envelopeAt(3); // start writes: sid1, sid2, sid3, then this
    ...
    const stop_id = try client.sendAgentStop(sid4, "done"); // stop@2, tracked
    var stopped_env = agent_types.Envelope{
        .session_id = sid4,
        .message_id = agent_types.generateUlid(),
        .sequence = 2,
    ...
    defer stopped_env.deinit(allocator);
    try client.processEnvelope(stopped_env);
    try std.testing.expect(!client.isSessionAdmitted(sid4)); // gone with the session
}
...
test "AgentProtocolClient correlated agent_not_found clears state only for a request of the current registration (#210 gap 7)" {
    ...
    // registration. A tracked send's not_found clears the stale counter and
    // control state — the ACTIVE IDENTITY survives (the TUI keys its
    // session sync on it; the clear lands with the drain-before-sync
    // rework, #224). A DELAYED not_found from an older registration's stop
    // — whose id the client never tracked, arriving after the reused id's
    // new agent_started — must leave the new registration's state alone,
    // or its next message would start at sequence 1 against the live
    // server session.
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    ...
    // the sequence and control state; the identity survives for the TUI's
    // drain (#224 owns its clear).
...
test "AgentProtocolClient correlated session_expired clears state like agent_not_found (#210 gap 7)" {
    // The idle-TTL eviction answer is as gone as not_found: the counter
    // and control state drop so a re-registration restarts at sequence 1,
    // and the admission slot drops so the eviction's stale evidence cannot
    // arm a later teardown stop against whatever registration occupies the
    // id next (§6.1's removed-and-re-registered bound). The identity
    // survives (the TUI drains the terminal error through it; #224 owns
    // the clear).
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    ...
    const sid = agent_types.generateSessionId();
    const start_id = try client.sendAgentStartWithSession(sid, "{}", null); // seq 1 (supplied id)
    var started_env = agent_types.Envelope{
        .session_id = sid,
        .message_id = agent_types.generateUlid(),
        .sequence = 1,
    ...
    defer started_env.deinit(allocator);
    try client.processEnvelope(started_env);
    // The correlation is recorded but the id is caller-supplied: not
    // admission (spec §6.1 — generation proof or an exclusive id).
    ...
    try std.testing.expect(!client.pending_sends_by_session.contains(sid));
    try std.testing.expect(client.session_id != null);
    try std.testing.expect(client.admitted_by_session.get(sid) == null); // slot cleared
    try std.testing.expect(!client.isSessionAdmitted(sid));
}
...
test "AgentProtocolClient plain stop replies reach the session-gone and rejection handling (#210 gap 7)" {
    ...
    // not_found answer to a plain stop clears the counter and control
    // state (the identity survives for the TUI's drain; #224 owns its
    // clear), and a rejection surfaces an observable failure.
    const allocator = std.testing.allocator;
    ...
    // Phase 1: the stop's agent_not_found answer clears the session state.
    {
        ...
        try std.testing.expectEqual(@as(u64, 1), client.peekNextSequence(sid));
        try std.testing.expect(!client.pending_sends_by_session.contains(sid));
        try std.testing.expect(client.session_id != null); // identity survives for the TUI's drain (#224)
    }
