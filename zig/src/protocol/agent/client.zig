...
const PendingSend = struct {
    ...
    /// still-unresolved original, so a later generic invalid_request
    /// rejection of it is duplicate evidence (an earlier copy was admitted;
...
pub const AgentProtocolClient = struct {
    ...
    pub fn sendAgentStopProbing(self: *Self, session_id: agent_types.SessionId, reason: ?[]const u8) !?agent_types.Ulid {
        ...
        for (list.items) |pending| {
            ...
            has_message = true;
            // The MINIMUM message sequence, not the first recorded: explicit
            // resends can be recorded out of order (a 999 before a 2), and
            // the floor must bracket the lowest reachable state — if both
            // admissions failed before advancing the server, every stop
            // above the minimum is too high. The MINIMUM pre-send tracker
            // matters for the same reason from the other side: a lone
            // FORWARD explicit send (session at 2, send at 999) records
            // prior 2 — if its rejection is lost, 2 is the one reachable
            // state below the optimistic values, and a floor without it
            // would probe only 999 and 1000 while the live session sits at
            // 2 (#210 gap 7).
        ...
        try candidates.append(self.allocator, tracker_value);
    ...
    pub fn processEnvelope(self: *Self, env: agent_types.Envelope) !void {
        switch (env.payload) {
            .agent_started => |p| {
                ...
                // send's unknown outcome against the session it created. The
                // evidence counts ONLY when the reply names this client's own
                // tracked start (the server correlates agent_started to the
                // start's message id): a delayed or unsolicited started from
                // an OLDER registration — or a FOREIGN caller's registration
                // that won a reused caller-supplied id — carries no ownership
                // proof, and recording it would let a later unknown-outcome
                // probe stop a session this client does not own. A lenient
                // peer that omits in_reply_to therefore never arms the probe;
                // callers that know their id is exclusive may still stop
                // explicitly (§13.2.6, sendAgentStopWithSequence).
                if (self.pendingSendKindFor(p.session_id, env.in_reply_to) == .start) {
            ...
            .agent_error => |e| {
                ...
                if (!try self.handleProbeReply(env.session_id, env.in_reply_to, e.code)) {
                    ...
                    // A generic invalid_request on a resend stays AMBIGUOUS:
                    // the agent surface collapses already-consumed and
                    // never-accepted mismatches into one code, and another
                    // pending record with the same sequence proves only that
                    // the original's outcome is UNRESOLVED — not that it was
                    // admitted. Converting it to admission evidence would
                    // suppress a real failure and pin the tracker above what
                    // the server accepts when the original was itself
                    // rejected. The ordinary rollback runs; the unknown side
                    // reconciles through the teardown probe's candidate
                    // bracket (#210 gap 7).
            ...
            .nack => |n| {
                ...
                if (!try self.handleProbeReply(env.session_id, env.in_reply_to, agentCodeFromNack(n.error_code))) {
                    ...
                    if (duplicate_admitted) {
                        // Only for a MESSAGE send, and only when it is a
                        // RETRY of a still-unresolved original
                        // (resend_of_pending): an earlier copy of the
                        // sequence was admitted, the code proves the server
                        // is past it, and the ORIGINAL's run may still
                        // settle — so the record retires with the pre-resend
                        // high-water and no session error. A duplicate
                        // answer on a NON-retry send admits nothing of this
                        // caller's own state: no run of the envelope will
                        // produce a settlement, so the high-water is
                        // preserved but the rejection SURFACES through the
                        // error bookkeeping below. A START answered
                        // duplicate_sequence — accepted with its started
                        // reply lost, then retransmitted — likewise falls
                        // through to the ordinary rejection path so the
                        // caller sees the failure and re-registers, leaking
                        // the accepted session only until the idle TTL
                        // (§13.2.6's conservative stop-ownership stance —
                        // claiming the accepted registration on duplicate
                        // evidence alone would also claim a FOREIGN caller's
                        ...
                        if (self.pendingSendFor(env.session_id, env.in_reply_to)) |entry| {
                            if (entry.kind == .message) {
                                ...
                                // The high-water is already preserved; skip
                                // the rollback (the entry is retired, so the
                                // correlated rejection below finds no match)
                                // and fall through to record the failure.
                            }
                        }
                    }
    ...
    fn handleProbeReply(self: *Self, session_id: agent_types.SessionId, in_reply_to: ?agent_types.Ulid, code: ?agent_types.AgentErrorCode) !bool {
        ...
        if (retry) {
            ...
            // REMAINING candidate slice AND the owned reason transfer to
            // the re-registered probe (same allocations, index advanced) —
            // no per-retry copies, and a later retry re-sends the caller's
            // reason rather than an empty one (#210 gap 7).
            try self.stop_probes_by_session.put(session_id, .{
    ...
    /// matches no tracked send for the session — callers that need to know
    /// WHICH of their own requests a reply resolves (e.g. that an
    /// agent_started really replies to a start, §6.1 admission evidence).
...
test "AgentProtocolClient probing stop tries pre-send first, then post-send on correlated invalid_request (#210 gap 7)" {
    ...
    // send only against a registration whose request-correlated
    // agent_started this observed.
...
test "AgentProtocolClient uncorrelated or foreign agent_started does not arm the stop probe (#210 gap 7)" {
    // A started with no request tie to this client's own tracked start
    // carries no ownership proof: on a caller-supplied id a foreign caller's
    // registration may be the one that started, and admission evidence from
    // that reply would let a later unknown-outcome probe stop the FOREIGN
    // session. Only the request-correlated started counts (the server ties
    // agent_started to the start's message id).
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    harness.wire();
    ...
    defer uncorrelated.deinit(allocator);
    try client.processEnvelope(uncorrelated);
    _ = try client.sendAgentMessage(sid1, "{\"m\":1}", null); // seq 2

    const probe1 = try client.sendAgentStopProbing(sid1, "timeout");
    try std.testing.expect(probe1 == null);
    try std.testing.expect(!client.admitted_by_session.contains(sid1));
    try std.testing.expectEqual(@as(usize, 2), harness.writes.items.len); // start + message only
    ...
    _ = try client.sendAgentMessage(sid2, "{\"m\":1}", null); // seq 2

    const probe2 = try client.sendAgentStopProbing(sid2, "timeout");
    try std.testing.expect(probe2 == null);
    try std.testing.expect(!client.admitted_by_session.contains(sid2));
    try std.testing.expectEqual(@as(usize, 4), harness.writes.items.len); // no stop in either phase
}
...
test "AgentProtocolClient correlated session_expired clears state like agent_not_found (#210 gap 7)" {
    ...
    // counter, and admission state drop so the next turn re-registers
    // instead of skipping agent_start and messaging into the void.
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    harness.wire();
...
test "AgentProtocolClient generic invalid_request on a resend stays ambiguous — ordinary rollback (#210 gap 7)" {
    ...
    // path: the tracker rolls to the rejected send's own sequence and the
    // failure surfaces; the unknown side reconciles through the teardown
    // probe's candidate bracket.
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    harness.wire();
...
test "AgentProtocolClient resend provenance survives the original's settlement (#210 gap 7)" {
    // The duplicate evidence for a generic invalid_request on a resend is
    // recorded ON THE RESEND (another unresolved message already carried the
    // sequence when it was sent), so the original's settlement processing —
    // which retires the original's record — does not erase it: the delayed
    // rejection still resolves as duplicate-admitted instead of rolling the
    // tracker back to the consumed sequence with a false terminal error.
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    harness.wire();
    ...
    // delayed rejection), retiring the oldest message record.
...
test "AgentProtocolClient probe floor includes the minimum pre-send tracker (#210 gap 7)" {
    // A lone FORWARD explicit send: session at 2, send at 999 (prior 2,
    // tracker 1000), its rejection lost. The reachable state 2 must anchor
    // the sweep — without the prior in the floor, the probe would carry
    // only 999 and 1000, both rejected, and teardown would discard local
    // state while the live session stays registered.
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    harness.wire();
    ...
    defer started_env.deinit(allocator);
    try client.processEnvelope(started_env);
    _ = try client.sendAgentMessageWithSequence(sid, "{\"m\":high}", null, 999); // prior 2, tracker 1000 — outcome lost

    const probe_result = try client.sendAgentStopProbing(sid, "timeout");
    const probe_stop_id = probe_result.?;
    var first_stop = try harness.envelopeAt(2); // start, m-high, floor stop
    defer first_stop.deinit(allocator);
    try std.testing.expectEqual(@as(u64, 2), first_stop.sequence); // the pre-send state anchors the sweep

    // The server never advanced past 2: the floor stop is ACCEPTED and the
    // session tears down.
    var stopped = agent_types.Envelope{
        .session_id = sid,
        .message_id = agent_types.generateUlid(),
    ...
    defer stopped.deinit(allocator);
    try client.processEnvelope(stopped);
    try std.testing.expectEqual(@as(usize, 3), harness.writes.items.len); // no retry needed
    try std.testing.expect(!client.stop_probes_by_session.contains(sid));
}

test "AgentProtocolClient probe retries carry the caller's reason (#210 gap 7)" {
    // The owned reason transfers to each re-registered probe, so every
    // retry stop — not just the first — reports why the session is being
    // torn down.
    const allocator = std.testing.allocator;
    var harness = Gap7Harness.init();
    defer harness.deinit();
    ...
    const probe_result = try client.sendAgentStopProbing(sid, "cancelled");
    var current_stop_id = probe_result.?;
    const expected = [_]u64{ 2, 3, 1000 };
    for (expected, 0..) |_, index| {
        ...
        try client.processEnvelope(rejection);
        if (index + 1 == expected.len) break;
        var stop = try harness.envelopeAt(3 + index + 1); // the retry this rejection produced
        defer stop.deinit(allocator);
        try std.testing.expectEqual(expected[index + 1], stop.sequence);
