...
/// Provider with context support for configuration
...
fn testStreamFnWithContext(
    ...
    const s = try allocator.create(event_stream.AssistantMessageStream);
    s.* = event_stream.AssistantMessageStream.init(allocator);
    return s;
}
...
test "Provider stream method" {
    ...
    const s = try p.stream(&messages, std.testing.allocator);
    defer {
        s.deinit();
...
test "Provider deinit with cleanup function" {
    const test_ctx = try std.testing.allocator.create(TestContext);
    test_ctx.* = TestContext{ .call_count = 0 };
...
test "Provider deinit without cleanup function" {
    const test_ctx = try std.testing.allocator.create(TestContext);
    defer std.testing.allocator.destroy(test_ctx);
    test_ctx.* = TestContext{ .call_count = 0 };
...
test "Registry unregister" {
    ...
    try registry.unregister("test-provider");
    try std.testing.expect(registry.get("test-provider") == null);
}
