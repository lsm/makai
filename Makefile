...
help:
	@echo "make build      build the makai CLI into zig-out/bin (MAKAI_CODESIGN_IDENTITY=<name> to sign)"
...
ifneq ($(MAKAI_CODESIGN_IDENTITY),)
	codesign --force --identifier com.makai.cli --sign "$(MAKAI_CODESIGN_IDENTITY)" zig-out/bin/makai
endif
